"""Graph + Vector + Fulltext hybrid retrieval.

Combines vector KNN, fulltext keyword search, and graph expansion
into a single merged result set. Matches the llm-graph-builder
`graph_vector_fulltext` chat mode.
"""
...
async def graph_vector_fulltext_search(
    ...
    records = await neo4j_store.graph_vector_fulltext_search(
    ...
    logger.info(
        "Graph+Vector+Fulltext search: %d results in %.1fms",
        len(chunks), latency_ms,
    )
    return chunks, latency_ms


def _parse_records(records: list[dict]) -> list[RetrievedChunk]:
    """Parse Neo4j hybrid records into RetrievedChunk models."""
    chunks = []
